...
def readNRCS(state):
    ...
    def getname(name):
        return df['site_name'].iloc[0].split('(')[0].strip()
    def gethucname(huc):
        return df['huc'].iloc[0].split('(')[0].strip()
    def gethucid(huc):
        return df['huc'].iloc[0].split('(')[1].split(')')[0]
    
    df['site_id'] = list(map(getid, df['site_name']))
    df['site_name'] = list(map(getname, df['site_name']))
    df['huc_name'] = list(map(gethucname, df['site_name']))
    df['huc_id'] = list(map(gethucid, df['site_name']))
    del df['huc']
    return df 
...
if __name__ == '__main__':
    engine = create_engine('sqlite:///snotel.db', echo=False)

    #logfile = 'snotel_download.log'
